...
// CRITICAL: Exit early if on Vercel - prevent ANY code from running
const isVercel = !!(typeof process !== 'undefined' && (process.env.VERCEL || process.env.VERCEL_ENV))

// If on Vercel, export no-op functions that do nothing
if (isVercel) {
  ...
  // Exit early - don't define any other code
} else {
  // Only define the actual worker code if NOT on Vercel
